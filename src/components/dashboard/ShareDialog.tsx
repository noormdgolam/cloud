...
// Radix's DialogContent unmounts children entirely while closed (no
// forceMount) — this component's state lives here rather than in
// ShareDialog itself so every reopen gets fresh initial state via a real
// mount, with no manual "reset on close" effect needed (that pattern trips
// react-hooks/set-state-in-effect for calling setState synchronously in an
// effect body; a mount-based reset is the fix React recommends instead).
function ShareDialogBody({ fileId }: { fileId: string }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title={`Share "${fileName}"`}>
        {open && <ShareDialogBody fileId={fileId} />}
      </DialogContent>
    </Dialog>
  );
}
